...
def get_candidate(db: Session, candidate_id: int, user: models.User):
    ...
    query = (
        ...
        .options(joinedload(models.Candidate.scores))
...
def get_candidates(
    db: Session, user: models.User, skip: int = 0, limit: int = 20, search: str = None
):
    query = db.query(models.Candidate).filter(
        models.Candidate.status != models.CandidateStatus.ARCHIVED
    )
...
def get_scores(db: Session, candidate_id: int, user: models.User):
    ...
    query = (
        ...
        .options(joinedload(models.Score.candidate))
...
def notify_score_update(
    db: Session, candidate_id: int, reviewer_id: int, score: int, category: str
):
    candidate = (
    ...
    if candidate and reviewer:
        ...
        logger.info(
            f"WS BROADCAST: Candidate {candidate.name}, Score {score}. Active Conns: {len(manager.active_connections)}"
        )
        ...
        logger.error(
            f"WS BROADCAST: Records not found (Cand: {candidate_id}, Rev: {reviewer_id})"
        )
...
def add_score(
    db: Session, candidate_id: int, score: schemas.ScoreCreate, reviewer_id: int
):
    existing_score = (
        db.query(models.Score)
        .filter(
    ...
    db_score = models.Score(
        **score.model_dump(), candidate_id=candidate_id, reviewer_id=reviewer_id
    )
    db.add(db_score)
    db.commit()
    db.refresh(db_score)
...
def generate_candidate_summary(
    db: Session, candidate_id: int, user: models.User, style: str = "concise"
):
    db_candidate = get_candidate(db, candidate_id, user)
    if not db_candidate:
        return None
    ...
    try:
        ...
        ai_logger.info(
            f"Ai Summary generate for this candidate {db_candidate.name} at this time"
        )
